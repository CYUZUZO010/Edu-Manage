...
import { BookOpen, Users, FileText, Upload, Calendar, CheckCircle, AlertCircle, LogOut } from "lucide-react"
...
export default function TeacherDashboard() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, subjectId: string) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        formData.append("teacherId", "teacher-1") // In real app, get from auth
        ...
        // Also save file metadata to API
      ...
      toast({
        ...
        description: `${files.length} file(s) uploaded successfully for ${subjects.find((s) => s.id === subjectId)?.name}!`,
      })
    } catch (error) {
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-white/20 shadow-sm">
        <div className="max-w-6xl mx-auto px-8 py-4">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={handleLogout} variant="outline" className="flex items-center space-x-2 hover:cursor-pointer">
      ...
      <div className="p-8">
        <div className="max-w-6xl mx-auto">
          <div className="mb-8 text-center">
            ...
            <p className="text-slate-600">Manage your classes and upload course materials</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200 hover:shadow-lg transition-all duration-300">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-purple-700">Files Uploaded</p>
          ...
          {/* Section Title */}
          <div className="mb-6">
            <h2 className="text-2xl font-semibold text-slate-800">Upload Course Materials</h2>
            <p className="text-slate-600">Manage and upload files for your subjects</p>
          </div>

          {/* Responsive Grid Layout - All Content */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Subject Cards */}
            {subjects.map((subject) => (
              <Card
                key={subject.id}
                className="hover:shadow-lg transition-all duration-300 bg-white/80 backdrop-blur-sm border border-white/30"
              >
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-4">
                      <div
                        className={`w-12 h-12 ${subject.color} rounded-lg flex items-center justify-center text-white font-semibold`}
                      >
                    ...
                    <div className="flex flex-col items-end space-y-3">
                      <Badge className="bg-slate-100 text-slate-800">{subject.students} students</Badge>

                      <Select
                        value={selectedCategory[subject.id] || "resource"}
                        onValueChange={(value) => handleCategoryChange(subject.id, value)}
                      >
            ...
            <Card className="bg-gradient-to-br from-green-50 to-green-100 border-green-200">
              <CardContent className="p-6">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-green-800">Upload Tips</h3>
                    <p className="text-sm text-green-700">
                      Select file category before uploading. Supported formats: PDF, DOC, PPT, images, ZIP
                    </p>
                  </div>
                </div>
              </CardContent>
