...
import { BookOpen, Download, FileText, Calendar, Search, LucideFile, RefreshCw, ExternalLink } from "lucide-react"
...
export default function StudentDashboard() {
  ...
  const fetchFiles = async (subjectId: string) => {
    try {
      ...
      setSubjects((prev) => prev.map((subject) => (subject.id === subjectId ? { ...subject, files: files } : subject)))
    ...
    <div className="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-600">Access your subjects and course materials</p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <Card className="bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200 hover:shadow-lg transition-all duration-300">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-blue-700">Enrolled Subjects</p>
              ...
              <Card className="bg-gradient-to-br from-emerald-50 to-emerald-100 border-emerald-200 hover:shadow-lg transition-all duration-300">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-emerald-700">Total Files</p>
                      <p className="text-2xl font-bold text-emerald-800">
                        {subjects.reduce((acc, subject) => acc + subject.files.length, 0)}
                      </p>
                    </div>
                ...
                >
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <Badge className="bg-slate-100 text-slate-800">{subject.files.length} files</Badge>
                    </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-800">{selectedSubjectData?.name}</h2>
                  <p className="text-slate-600">{selectedSubjectData?.teacher}</p>
                </div>
              </div>
              ...
              {loading ? (
                <Card className="bg-white/80 backdrop-blur-sm border border-white/30">
                  <CardContent className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-slate-800 mb-2">Loading files...</h3>
                    <p className="text-slate-600">Please wait while we fetch the latest files</p>
                  </CardContent>
                </Card>
              ) : (
                ...
                <Card className="bg-white/80 backdrop-blur-sm border border-white/30">
                  <CardContent className="text-center py-12">
                    ...
                    <p className="text-slate-600">
                      {searchItem ? "Try adjusting your search terms" : "No files have been uploaded yet"}
                    </p>
                  </CardContent>
                </Card>
              )}
